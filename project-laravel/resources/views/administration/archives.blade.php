...
@section('page-header')
				<!-- breadcrumb -->
                <?php $etat='';$cnt=0;?>
                @foreach($l as $st2)<?php $etat=$st2->status ;$cnt=$cnt+1;?>    @endforeach
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">
                          <?php if($etat=='تم الغاء الطلب'){ echo "صفحة الطلبات الملغاة".
                             "</h4><span class='text-muted mt-1 tx-13 mr-2 mb-0'> 
                             <span class='badge badge-danger'>".$cnt."</span>" ;}?>
                          <?php if($etat=='مرسلة بنجاح'){ echo "صفحة الطلبات المرسلة بنجاح".
                            "</h4><span class='text-muted mt-1 tx-13 mr-2 mb-0'> 
							...
							<div class="card-body">
								...
								<table class="table table-hover text-md-nowrap" id="example1">
										<thead>
                                            ...
                                            <?php if($etat=='مرسلة بنجاح'):?>
											<th class="wd-15p border-bottom-0">Imprimer</th>
                                            <?php endif;?>
											</tr>
										</thead>
										<tbody>
...
<div class="modal fade "  id="commande<?php echo $t;?>">
			...
			@if(Session::has('coin'))  {{Session::get('coin')}}@endif</h4>
											...
											<?php if($etat=='مرسلة بنجاح'):?>
											<td  >
											<span class="badge bg-success-transparent text-success mr-auto ">
											<?php echo $st->status ;?>...</span>	
                                             </td>
											 <?php else:?>
											<td  >
											<span class="badge bg-danger-transparent text-danger mr-auto ">
											<?php echo $st->status ;?>...</span>	
                                             </td>
											 <?php endif;?>
                                            ...
                                            <?php if($etat=='مرسلة بنجاح'):?>
                                                ...
                                                <a href="#imprimer"  class="btn btn-info  btn-sm "
                                                 class="modal-effect "
		...
		<div class="modal effect-fall show" id="imprimer">
			<div class="modal-dialog modal-dialog-centered" role="document">
...
<div style="direction: ltr;" id="date" ></div>
					...
					<div class="col-12" id="total"></div>
				     ...
				     <button class="btn ripple btn-info btn-block"  onclick="imprimer()"
                      data-dismiss="modal" type="button">
                     <i class="mdi mdi-printer tx-22"></i>
                     </button>
				</div>
			</div>
		</div>
...
document.getElementById("total").innerHTML = "<table class='table table-bordered'><tr><td>المبلغ الاجملي :</td><td class='text-center'>"+total+" @if(Session::has('coin'))  {{Session::get('coin')}}@endif </td></tr></table>";
});
function imprimer(){
