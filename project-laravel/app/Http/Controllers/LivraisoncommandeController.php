...
class LivraisoncommandeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('administration.livraison-commandes',compact('s','tcc','tc'));
    }
    ...
    public function update(Request $request)
    {
        if(isset($request['id'])){
        $id=$request['id'];
        DB::update("UPDATE commande SET status='مرسلة بنجاح' WHERE id='$id'");
        DB::update("UPDATE  statistique SET etat='مرسلة بنجاح' WHERE cmd=(SELECT commande.matricule FROM commande WHERE commande.id='$id')");
